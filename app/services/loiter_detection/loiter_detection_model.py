...
from core.main.export_data_v3 import export_data_loiter_detection, export_data_intrusion_detection
from core.main.drawing import drawing_loitering
...
class InfoCam(object):
    def __init__(self, cam_name, process_name, info_cam_running, test_video):
        ...
        self.waiting_time = info_cam_running["waiting_time"]  # 10s
        print("waiting_time: ", self.waiting_time)
        coordinates = ast.literal_eval(info_cam_running["coordinates"])
        coordinates = convert_coordinates(coordinates, self.width, self.height)
        self.region_track = list([coordinates[0], coordinates[1], coordinates[2], coordinates[3]])
...
def run_loiter(item_cam, end_time_s, info_cam_running, test_video=False, cv2_show=True, window="test"):
    ...
    thread7 = KThread(target=export_data_loiter_detection, args=(cam, database_queue, object_dal))
    # thread7 = KThread(target=export_data_intrusion_detection, args=(cam, database_queue, object_dal))
    # thread8 = KThread(target=drawing, args=(cam, show_queue, show_all_queue, frame_final_queue))
    thread8 = KThread(target=drawing_loitering, args=(cam, show_all_queue, frame_final_queue))
