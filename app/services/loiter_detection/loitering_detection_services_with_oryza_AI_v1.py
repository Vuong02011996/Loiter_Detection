...
from app.services.loiter_detection.loiter_detection_model import run_loiter

blueprint = Blueprint("Loiter_Service", __name__)
# Using register_blueprint
api = Api(blueprint, title="Loiter_Service", version="1.0.0", description="Swagger API Homepage")
...
object_dal = ObjectLoiteringDAL()
process_dal = ProcessLoiteringDAL()
...
class StartById(Resource):
    def post(self):
        ...
        # Create new process
        process_name = id_cam_mg + "_loiter_" + str(len(process_dal.find_all_item())) + "_" + \
        ...
        try:
            def process_data():
                ...
                process = mp.Process(target=run_loiter,
        ...
        if start_server_for_run_loop:
            ...
            def process_loop_send_status():
                ...
                while True:
                    ...
                    data_send = {'ip': ip, 'port': os.getenv("SERVER_PORT_LOITERING"),
                                 'data': info_data}
                    pikaPublisher = PikaPublisher()
