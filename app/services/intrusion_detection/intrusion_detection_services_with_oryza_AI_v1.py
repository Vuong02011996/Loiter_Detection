...
from app.services.intrusion_detection.intrusion_detection_model import run_intrusion

blueprint = Blueprint("Intrusion_Service", __name__)
# Using register_blueprint
api = Api(blueprint, title="Intrusion_Service", version="1.0.0", description="Swagger API Homepage")
...
object_dal = ObjectIntrusionDAL()
process_dal = ProcessIntrusionDAL()
...
class StartById(Resource):
    def post(self):
        ...
        Post json data:
        {
            "process_id": "9a5dcef8-8028-5c36-56b9-ee51381f454d2",
            "rtsp": "rtsp://admin:oryza@2023@192.168.111.63:7001/9a5dcef8-8028-5c36-56b9-ee51381f454d",
            "ip_camera": "192.168.103.25",
            "boundary": "[[0.36, 0.34], [0.67, 0.39], [0.71, 0.86], [0.32, 0.91]]",
            "waiting_time": 10
        }
        boundary: Tọa độ vùng nhận diện lảng vảng
        waiting_time: Thời gian (s)người ở trong vùng lảng vảng sẽ được gửi event.
        Returns:
        ...
        coordinates = request_data.get("boundary")
        waiting_time = request_data.get("waiting_time")
        is_debug = request_data.get("is_debug")
        print("rtsp: ", rtsp)
        print("is_debug: ", is_debug)
        print("id_camera: ", id_camera)
        print("ip_camera: ", ip_camera)
        print("coordinates boundary: ", coordinates)
        ...
        if start_server:
            for p in list_all_item_process:
                if p["status_process"] != "Stop":
                    data_update = {
                        "status_process": "Stop",
                    }
                    process_dal.update({"_id": p["_id"]}, data_update)
            start_server = False
        ...
        # if item_cam.get("jobs_cam") is None or item_cam["jobs_cam"].get("intrusion") is None:
        ...
        # Create new process
        process_name = id_cam_mg + "_intrusion_" + str(len(process_dal.find_all_item())) + "_" + \
        ...
        try:
            def process_data():
                ...
                process = mp.Process(target=run_intrusion,
            ...
            print("Setup camera error , can't start attendance on camera")
            return options
        ...
        if start_server_for_run_loop:
            ...
            def process_loop_send_status():
                ...
                while True:
                    ...
                    data_send = {'ip': ip, 'port': os.getenv("SERVER_PORT_INTRUSION"),
                                 'data': info_data}
                    pikaPublisher = PikaPublisher()
