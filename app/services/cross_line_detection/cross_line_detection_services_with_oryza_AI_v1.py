...
from app.services.cross_line_detection.cross_line_detection_model import run_cross_line

blueprint = Blueprint("Cross_Line_Service", __name__)
# Using register_blueprint
api = Api(blueprint, title="Cross_Line_Service", version="1.0.0", description="Swagger API Homepage")
...
class StartById(Resource):
    def post(self):
        ...
        Post json data:
        {
            ...
            "rtsp": "rtsp://digesttest2:Oryza@123@192.168.111.59:7001/9a5dcef8-8028-5c36-56b9-ee51381f454d",
            "ip_camera": "192.168.111.63",
            "coordinates_lines": "[[0.399, 0.421], [0.496, 0.73]]"
        }
        coordinates_lines: Tọa độ line
        Returns:
        ...
        coordinates_lines = request_data.get("line")
        ...
        port_model_head = request_data.get("port_model_head")
        port_model_face = request_data.get("port_model_face")
        ...
        for p in list_all_item_process:
            if p["process_name"].split("_")[0] == id_cam_mg and p["status_process"] == "running" and (
                    "cross_line" in p["process_name"]):
                print("Camera is running this service, process_name is {}".format(p["process_name"]))
        ...
        # if item_cam.get("jobs_cam") is None or item_cam["jobs_cam"].get("loiter") is None:
        ...
        # Create new process
        process_name = id_cam_mg + "_cross_line_" + str(len(process_dal.find_all_item())) + "_" + \
        ...
        try:
            def process_data():
                ...
                process = mp.Process(target=run_cross_line,
        ...
        if start_server_for_run_loop:
            ...
            def process_loop_send_status():
                ...
                while True:
                    ...
                    data_send = {'ip': ip, 'port': os.getenv("SERVER_PORT_CROSS_LINE"),
                                 'data': info_data}
                    pikaPublisher = PikaPublisher()
