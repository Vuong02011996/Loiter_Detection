...
def draw_boxes_one_track_id(image, track_bbs_ids, track_id,  line_thickness=1, font_scale=1.0,
    ...
    for idx, b in enumerate(track_bbs_ids):
        if int(current_track_id[idx]) == track_id:

            xmin, ymin, xmax, ymax, track_id_ = list(map(int, b))
            print("track_id_: ", track_id_)
...
def draw_boxes_one_track_id(image, box_of_track, track_id,  line_thickness=1, font_scale=1.0,
                        font_thickness=2, color=(255, 0, 0)):
    """
    ...
    """
    xmin, ymin, xmax, ymax = box_of_track
    cv2.rectangle(image, (xmin, ymin), (xmax, ymax), color, line_thickness)
    # draw bbox face
    # put id track to image
    # print("track_id inside 2: ", track_id)
    cv2.putText(image, str(int(track_id)), (xmin, ymin + 10), cv2.FONT_HERSHEY_SIMPLEX, 2, color,2)
