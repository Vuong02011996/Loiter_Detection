...
object_dal = ObjectSafeRegionDAL()
...
def matching_identity(cam, matching_queue, database_queue, show_queue):
    while cam.cap.isOpened():
        face_embeddings, track_bbs_ids, frame_count, frame_rgb, boxes_face = matching_queue.get()
        matching_info = [{
                        "name": "Unknown",
                        "url_match": None,
                        "identity_id": None,
        ...
        if len(face_embeddings) > 0:
            ...
            for idx in range(len(match_identity)):
                if match_identity[idx]["distance"] < 0.45:  # 0.45
        ...
        database_queue.put([track_bbs_ids, matching_info, frame_count, frame_rgb, boxes_face])
