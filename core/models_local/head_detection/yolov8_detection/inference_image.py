...
path_model_head_person = '/home/oryza/Desktop/Projects/Loiter_Detection/core/models_local/head_detection/yolov8_detection/only_person_22_8.pt'
# path_model_head_person = "/home/oryza/Desktop/Projects/Loiter_Detection/core/models_local/head_detection/yolov8_detection/model_a_T_v1.pt"
# model = YOLO('models/model_Manh.pt')
model = YOLO(path_model_head_person)
...
def infer_2():
    image_path = '/home/oryza/Desktop/Projects/ai-engineer/Model_Yolo/images_test/body_test.png'
...
def person_detect(image_infer):
    # model = YOLO('models/model_Manh.pt')

    result = model(image_infer, verbose=False, conf=0.4, iou=0.4)
    bboxes = []
    labels = []
    scores = []
    preds = []
    for det in result[0].boxes:
        ...
        if cls == 0.0:
            # xmin, ymin, xmax, ymax = det.xyxy[0].tolist()  # Convert tensor to list for easier handling
            x1, y1, x2, y2 = map(int, det.xyxy[0].tolist())  # Convert tensor to list and cast to int
            conf = det.conf.item()  # Get Python number from tensor
            # print("x1: ", x1, "y1: ", y1, "x2: ", x2, "y2: ", y2, "cls: ", cls, "conf: ", conf)
            bboxes.append(list(map(int, [x1, y1, x2, y2])))
            labels.append('body')
            scores.append(float(conf))
            preds.append(np.array([int(x1), int(y1), int(x2), int(y2), float(conf), int(cls)]))
